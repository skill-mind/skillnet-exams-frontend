...
import { SendIcon, SmileIcon, PaperclipIcon, ArrowLeftIcon } from "lucide-react"
...
const QuestionChat = ({ question, onBack, isMobile }: QuestionChatProps) => {
  const [messages, setMessages] = useState<MessageType[]>(question.messages || [])
  ...
  return (
    <>
      ...
      <div className="flex items-center p-3 border-b border-[#333]">
        {isMobile && (
          <button onClick={onBack} className="mr-2 p-1.5 rounded-lg text-gray-400 hover:text-white">
      ...
      <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto">
        {messages.map((message) => (
          <div key={message.id} className={`flex mb-4 ${message.sender === "user" ? "justify-start" : "justify-end"}`}>
            <div
              className={`max-w-[80%] rounded-2xl p-3 ${
                message.sender === "user" ? "bg-[#252525]" : "bg-[#8b5cf6] text-white"
              }`}
            >
              <p className="text-sm">{message.content}</p>
              <div className="flex justify-end mt-1">
                <span className={`text-xs ${message.sender === "user" ? "text-gray-400" : "text-gray-200"}`}>
                  {message.timestamp}
                </span>
              </div>
            </div>
      ...
      <div className="p-3 border-t border-[#333]">
        <div className="flex items-center bg-[#252525] rounded-full p-1">
          ...
          <button
            ...
            className={`p-2 rounded-full ${inputMessage.trim() ? "text-white bg-green-500" : "text-gray-400"}`}
          >
            <SendIcon size={18} />
          </button>
        </div>
